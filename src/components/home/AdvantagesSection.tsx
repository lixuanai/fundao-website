    ...
    <section className="py-12 sm:py-16 md:py-24 relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8 sm:mb-12 md:mb-16">
          <span className="tag-purple mb-3 sm:mb-4 inline-block text-xs sm:text-sm">{tBadge('text')}</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {advantages.map((item, i) => (
            <div
              key={i}
              className={`relative bg-gradient-to-br ${item.bgGradient} rounded-xl sm:rounded-2xl p-5 sm:p-6 md:p-8 card-hover border ${item.borderColor} group overflow-hidden`}
            >
              {/* Icon */}
              <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-br ${item.gradient} flex items-center justify-center mb-4 sm:mb-6 text-white shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                {item.icon}
              </div>
